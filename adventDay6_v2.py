...
def problem1(path):
	...
	while True:
		...
		limit = currBank[maxBank]
		currBank[maxBank] = 0
		
		i = evaluateLimit(len(currBank), maxBank) 
		for k in range(0, limit):
			currBank[i] += 1
			i = evaluateLimit(len(currBank), i)
...
def evaluateLimit(lenBankArr, currIndex):
	if currIndex < (lenBankArr - 1):
		i = currIndex + 1
	else:
		i = 0
	return i
